...
*/
// PATCHED for Blender port: threading is opt-in via -DSYNTH_ENABLE_THREADS.
// Default is single-threaded: deterministic results, no pthread dependency on Windows.
#ifdef SYNTH_ENABLE_THREADS
#define SYNTH_THREADED TRUE
#endif
// If not defined, uses POSIX threads.  Moot unless SYNTH_THREADED
#if defined(SYNTH_THREADED) && !defined(_WIN32) && !defined(_WIN64) && !defined(SYNTH_LIB_ALONE)
